...
def process_image_to_3d(image_path, output_path):
    ...
    # 2. Extract Contours
    # We use approximation to keep the model light
    contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    
    scene = trimesh.Scene()
    
    # Floor (Dark Gray) - Gives context to the model
    h, w = img.shape
    ...
    # 3. Build Walls using Extrusion
    for cnt in contours:
        # Simplify contour
        epsilon = 0.005 * cv2.arcLength(cnt, True)
        approx = cv2.approxPolyDP(cnt, epsilon, True)
        
        # We need at least 3 points to make a shape
        if len(approx) >= 3:
            points = approx.squeeze()
            
            try:
                # Create a Shapely Polygon
                # This works reliably without Scipy
                poly = Polygon(points)
                
                if poly.is_valid and poly.area > 50:
                    # Extrude
                    wall_mesh = trimesh.creation.extrude_polygon(poly, height=wall_height)
                    wall_mesh.visual.face_colors = [240, 240, 240, 255] # Off-White Walls
                    scene.add_geometry(wall_mesh)
            except Exception as e:
                print(f"Skipping invalid shape: {e}")

    # 4. Export as GLB
    # This ensures it works on your Mobile UI frontend
